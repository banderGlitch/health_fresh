...
```bash
curl -X POST http://localhost:8002/analyze/continue \
  -H "Content-Type: application/json" \
  -d '{
...
```powershell
# 1. Analyze
$r1 = Invoke-RestMethod -Uri "http://localhost:8002/analyze" -Method POST `
...
$r2 = Invoke-RestMethod -Uri "http://localhost:8002/analyze/continue" -Method POST `
  -ContentType "application/json" `
...
curl -X POST http://localhost:8002/analyze/continue -H "Content-Type: application/json" \
