"""
Map pipeline features to finetuned triage model input format.

Symptom mapping: NER output (symptom names) -> one of 20 triage categories.
Feature mapping: age -> 16-30|31-50|50+|6-15, duration -> 1-2 days|3-5 days|1 week|2+ weeks.
"""

from typing import Any

# 20 symptom categories (from label_encoders.pkl)
TRIAGE_SYMPTOMS = [
    "Breathing shortness",
...
# Keyword rules: (symptom_keywords_set, severity_hint) -> triage category
...
def features_to_triage_input(features: dict[str, Any]) -> dict[str, Any] | None:
    ...
    symptom_category = map_symptoms_to_category(
        symptom_names, severities, max_days
    )
